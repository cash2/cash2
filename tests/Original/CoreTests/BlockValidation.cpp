...
namespace {
  bool lift_up_difficulty(const CryptoNote::Currency& currency, std::vector<test_event_entry>& events,
                          ...
                          const CryptoNote::AccountBase& miner_account) {
    ...
    for (size_t i = 0; i < new_block_count; ++i) {
      ...
      CryptoNote::difficulty_type diffic = currency.nextDifficulty1(timestamps, cummulative_difficulties);
      if (!generator.constructBlockManually(blk_next, blk_prev, miner_account,
        test_generator::bf_timestamp | test_generator::bf_diffic, 
        blk_prev.timestamp, Crypto::Hash(), diffic)) {
        return false;
      }
...
bool gen_block_ts_not_checked::generate(std::vector<test_event_entry>& events) const
{
  ...
  generator.constructBlockManually(blk_1, blk_0r, miner_account, 
    test_generator::bf_timestamp, blk_0.timestamp - 60 * 60);
...
bool gen_block_ts_in_past::generate(std::vector<test_event_entry>& events) const
{
  ...
  Block blk_1;
  generator.constructBlockManually(blk_1, blk_0r, miner_account, 
    test_generator::bf_timestamp, ts_below_median);
  events.push_back(blk_1);
...
bool gen_block_invalid_prev_id::generate(std::vector<test_event_entry>& events) const
{
  ...
  generator.constructBlockManually(blk_1, blk_0, miner_account, 
    test_generator::bf_prev_id, 0, prev_id);
  events.push_back(blk_1);
...
bool gen_block_no_miner_tx::generate(std::vector<test_event_entry>& events) const
{
  ...
  Block blk_1;
  generator.constructBlockManually(blk_1, blk_0, miner_account, 
    test_generator::bf_miner_tx, 0, Crypto::Hash(), 0, miner_tx);
  events.push_back(blk_1);
...
bool gen_block_miner_tx_has_2_in::generate(std::vector<test_event_entry>& events) const
{
  ...
  if (!constructTransaction(miner_account.getAccountKeys(), sources, destinations, std::vector<uint8_t>(), tmp_tx, 0, transactionSecretKeyIgnore, m_logger))
    return false;
...
bool gen_block_invalid_binary_format::generate(std::vector<test_event_entry>& events) const
{
  ...
  do
  {
    ...
    diffic = m_currency.nextDifficulty1(timestamps, cummulative_difficulties);
  ...
  if (!generator.constructBlockManually(blk_test, blk_last, miner_account,
    test_generator::bf_diffic | test_generator::bf_timestamp | test_generator::bf_tx_hashes, 
    blk_last.timestamp, Crypto::Hash(), diffic, Transaction(), tx_hashes, txs_size))
    return false;
