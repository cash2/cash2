...
bool gen_ring_signature_1::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_TX_LIST_START(events, txs_blk_6, miner_account, bob_account, 100000000, blk_5);               //  9 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, bob_account, 1100000000 + rnd_11, blk_5);           // 10 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, bob_account, 1100000000 + rnd_11, blk_5);           // 11 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, bob_account, 2000000000 + rnd_20, blk_5);           // 12 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, bob_account, 2900000000 + rnd_29, blk_5);           // 13 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, bob_account, 2900000000 + rnd_29, blk_5);           // 14 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, bob_account, 2900000000 + rnd_29, blk_5);           // 15 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, some_account_1, 1100000000 + rnd_11, blk_5);        // 16 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, some_account_1, 1100000000 + rnd_11, blk_5);        // 17 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, some_account_1, 1100000000 + rnd_11, blk_5);        // 18 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, some_account_1, 1100000000 + rnd_11, blk_5);        // 19 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, some_account_1, 2000000000 + rnd_20, blk_5);        // 20 + 2N
  MAKE_TX_LIST(events, txs_blk_6, miner_account, some_account_2, 2000000000 + rnd_20, blk_5);        // 21 + 2N
  ...
  MAKE_TX_MIX(events, tx_0, bob_account, alice_account, 12900000000 + 2 * rnd_11 + rnd_20 + 3 * rnd_29 - m_currency.minimumFee(), 2, blk_6);  // 24 + 3N
...
bool gen_ring_signature_1::check_balances_1(CryptoNote::Core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  CHECK_TEST_CONDITION(r);
  CHECK_EQ(13000000000 + 2 * rnd_11 + rnd_20 + 3 * rnd_29, get_balance(m_bob_account, chain, mtx));
...
bool gen_ring_signature_1::check_balances_2(CryptoNote::Core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  r = find_block_chain(events, chain, mtx, get_block_hash(blocks.back()));
  CHECK_TEST_CONDITION(r);
  CHECK_EQ(13003711771, get_balance(m_bob_account, chain, mtx));
  CHECK_EQ(12900000000 + 2 * rnd_11 + rnd_20 + 3 * rnd_29 - m_currency.minimumFee(), get_balance(m_alice_account, chain, mtx));
...
bool gen_ring_signature_2::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_TX_LIST_START(events, txs_blk_4, miner_account, bob_account, 610000000, blk_3);               //  6 + N
  MAKE_TX_LIST(events, txs_blk_4, miner_account, bob_account, 610000000, blk_3);                     //  7 + N
  MAKE_TX_LIST(events, txs_blk_4, miner_account, bob_account, 610000000, blk_3);                     //  8 + N
  MAKE_TX_LIST(events, txs_blk_4, miner_account, bob_account, 610000000, blk_3);                     //  9 + N
  ...
  MAKE_TX_MIX(events, tx_0, bob_account, alice_account, 2440000000 - m_currency.minimumFee(), 3, blk_4);   // 12 + 2N
...
bool gen_ring_signature_2::check_balances_1(CryptoNote::Core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  r = find_block_chain(events, chain, mtx, get_block_hash(blocks.back()));
  CHECK_TEST_CONDITION(r);
  CHECK_EQ(2440000000, get_balance(m_bob_account, chain, mtx));
  CHECK_EQ(0, get_balance(m_alice_account, chain, mtx));
...
bool gen_ring_signature_2::check_balances_2(CryptoNote::Core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  CHECK_EQ(2440000000 - m_currency.minimumFee(), get_balance(m_alice_account, chain, mtx));
