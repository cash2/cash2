...
#define CURRENT_BLOCKCHAININDEXES_STORAGE_ARCHIVE_VER 1

namespace CryptoNote {
class BlockCacheSerializer;
class BlockchainIndexesSerializer;
}
...
public:
  BlockchainIndexesSerializer(Blockchain& bs, const Crypto::Hash lastBlockHash, ILogger& logger) :
    m_bs(bs), m_lastBlockHash(lastBlockHash), m_loaded(false), logger(logger, "BlockchainIndexesSerializer") {
  }

  void serialize(ISerializer& s) {

    uint8_t version = CURRENT_BLOCKCHAININDEXES_STORAGE_ARCHIVE_VER;

    KV_MEMBER(version);

    // ignore old versions, do rebuild
    if (version != CURRENT_BLOCKCHAININDEXES_STORAGE_ARCHIVE_VER)
      return;
...
bool Blockchain::switch_to_alternative_blockchain(std::list<blocks_ext_by_hash::iterator>& alt_chain, bool discard_disconnected_chain) {
  ...
  size_t split_height = alt_chain.front()->second.block_index;
...
difficulty_type Blockchain::get_next_difficulty_for_alternative_chain(const std::list<blocks_ext_by_hash::iterator>& alt_chain, BlockEntry& bei) {
  ...
  if (alt_chain.size() < m_currency.difficultyBlocksCount()) {
    std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
    size_t main_chain_stop_offset = alt_chain.size() ? alt_chain.front()->second.block_index : bei.block_index;
    ...
    for (; main_chain_start_offset < main_chain_stop_offset; ++main_chain_start_offset) {
      ...
      cummulative_difficulties.push_back(m_blocks[main_chain_start_offset].cumulative_difficulty);
    }
    ...
    for (auto it : alt_chain) {
      ...
      cummulative_difficulties.push_back(it->second.cumulative_difficulty);
    }
  } else {
    timestamps.resize(std::min(alt_chain.size(), m_currency.difficultyBlocksCount()));
    cummulative_difficulties.resize(std::min(alt_chain.size(), m_currency.difficultyBlocksCount()));
    size_t count = 0;
    size_t max_i = timestamps.size() - 1;
    BOOST_REVERSE_FOREACH(auto it, alt_chain) {
      timestamps[max_i - count] = it->second.bl.timestamp;
      cummulative_difficulties[max_i - count] = it->second.cumulative_difficulty;
      count++;
  ...
  if (bei.block_index < parameters::HARD_FORK_HEIGHT_2)
  {
    return m_currency.nextDifficulty1(timestamps, cummulative_difficulties);
  }
  else if (bei.block_index < parameters::HARD_FORK_HEIGHT_2 + 4000)
  {
    // loading the cummualtive_difficulties values in each block with 500 for 4000 blocks
    // so that afterwards the block difficulties can be calculated using cumulative difficulties
    return 500;
...
bool Blockchain::handle_alternative_block(const Block& b, const Crypto::Hash& id, block_verification_context& bvc, bool sendNewAlternativeBlockMessage) {
  ...
  if (it_prev != m_alternative_chains.end() || mainPrev) {
    ...
    if (alt_chain.size()) {
      ...
      if (!(m_blocks.size() > alt_chain.front()->second.block_index)) { logger(ERROR, BRIGHT_RED) << "main blockchain wrong height"; return false; }
      Crypto::Hash h = NULL_HASH;
      get_block_hash(m_blocks[alt_chain.front()->second.block_index - 1].bl, h);
      ...
      complete_timestamps_vector(alt_chain.front()->second.block_index - 1, timestamps);
    } else {
    ...
    bei.bl = b;
    bei.block_index = static_cast<uint32_t>(alt_chain.size() ? it_prev->second.block_index + 1 : mainPrevHeight + 1);
    ...
    if (bei.block_index < parameters::HARD_FORK_HEIGHT_2 && getCurrentBlockchainHeight() >= parameters::HARD_FORK_HEIGHT_2)
    {
    ...
    if (!m_checkpoints.check_block(bei.block_index, id, is_a_checkpoint)) {
      logger(ERROR, BRIGHT_RED) <<
    ...
    if (!prevalidate_miner_transaction(b, bei.block_index)) {
      logger(INFO, BRIGHT_RED) <<
        "Block with id: " << Common::podToHex(id) << " (as alternative) have wrong miner transaction.";
      bvc.m_verification_failed = true;
      return false;
    ...
    if (is_a_checkpoint) {
      //do reorganize!
      logger(INFO, BRIGHT_GREEN) <<
        "###### REORGANIZE on height: " << alt_chain.front()->second.block_index << " of " << m_blocks.size() - 1 <<
        ", checkpoint is found in alternative chain on height " << bei.block_index;
    ...
    else if (m_blocks.back().cumulative_difficulty < bei.cumulative_difficulty) //check if difficulty bigger then in main chain
    {
      //do reorganize!
      logger(INFO, BRIGHT_GREEN) <<
        "###### REORGANIZE on height: " << alt_chain.front()->second.block_index << " of " << m_blocks.size() - 1 << " with cum_difficulty " << m_blocks.back().cumulative_difficulty
      ...
      logger(INFO, BRIGHT_BLUE) <<
        "----- BLOCK ADDED AS ALTERNATIVE ON HEIGHT " << bei.block_index
        << ENDL << "id:\t" << id
...
bool Blockchain::getBlockCumulativeDifficulty(uint32_t blockIndex, uint64_t& cumulativeDifficulty) {
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
  if (blockIndex >= m_blocks.size())
  {
    logger(ERROR, BRIGHT_RED) << "Wrong block index = " << blockIndex << " at Blockchain::getBlockCumulativeDifficulty()";
    return false;
  }

  cumulativeDifficulty = m_blocks[blockIndex].cumulative_difficulty;
  return true;
}
...
bool Blockchain::getTransactionOutputGlobalIndexes(const Crypto::Hash& tx_id, std::vector<uint32_t>& indexes) {
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
  auto it = m_transactionMap.find(tx_id);
  if (it == m_transactionMap.end()) {
    logger(WARNING, YELLOW) << "warning: get_tx_outputs_gindexes failed to find transaction with id = " << tx_id;
    return false;
  }
  ...
  indexes.resize(tx.m_global_output_indexes.size());
  for (size_t i = 0; i < tx.m_global_output_indexes.size(); ++i) {
    indexes[i] = tx.m_global_output_indexes[i];
  }
...
bool Blockchain::check_block_timestamp_main(const Block& b) {
  if (b.timestamp > get_adjusted_time() + m_currency.blockFutureTimeLimit()) {
    logger(INFO, BRIGHT_WHITE) <<
      "Timestamp of block with id: " << get_block_hash(b) << ", " << b.timestamp << ", bigger than adjusted time + 27 seconds";
    return false;
  }
...
bool Blockchain::pushBlock(const Block& blockData, const std::vector<Transaction>& transactions, block_verification_context& bvc) {
  ...
  block.block_index = static_cast<uint32_t>(m_blocks.size());
  ...
  logger(DEBUGGING) <<
    ...
    << ENDL << "PoW:\t" << proof_of_work
    << ENDL << "HEIGHT " << block.block_index << ", difficulty:\t" << currentDifficulty
...
bool Blockchain::getBlockContainingTransaction(const Crypto::Hash& txId, Crypto::Hash& blockId, uint32_t& blockHeight) {
  ...
  if (it == m_transactionMap.end()) {
    ...
    blockHeight = m_blocks[it->second.block].block_index;
...
bool Blockchain::storeBlockchainIndexes() {
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);

  logger(INFO, BRIGHT_WHITE) << "Saving blockchain indexes...";
  BlockchainIndexesSerializer ser(*this, getTailId(), logger.getLogger());

  if (!storeToBinaryFile(ser, appendPath(m_config_folder, m_currency.blockchainIndexesFileName()))) {
    logger(ERROR, BRIGHT_RED) << "Failed to save blockchain indexes";
    return false;
  }
...
bool Blockchain::loadBlockchainIndexes() {
  std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);

  logger(INFO, BRIGHT_WHITE) << "Loading blockchain indexes for BlockchainExplorer...";
  BlockchainIndexesSerializer loader(*this, get_block_hash(m_blocks.back().bl), logger.getLogger());

  loadFromBinaryFile(loader, appendPath(m_config_folder, m_currency.blockchainIndexesFileName()));

  if (!loader.loaded()) {
    logger(WARNING, BRIGHT_YELLOW) << "No actual blockchain indexes for BlockchainExplorer found, rebuilding...";
    std::chrono::steady_clock::time_point timePoint = std::chrono::steady_clock::now();
    ...
    std::chrono::duration<double> duration = std::chrono::steady_clock::now() - timePoint;
    logger(INFO, BRIGHT_WHITE) << "Rebuilding blockchain indexes took: " << duration.count();
  }
  return true;
}
