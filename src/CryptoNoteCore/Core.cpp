...
std::vector<Crypto::Hash> Core::findBlockchainSupplement(const std::vector<Crypto::Hash>& remoteBlockIds, size_t maxCount, uint32_t& totalBlockCount, uint32_t& startBlockIndex) {
...
uint64_t Core::getMinimalFee() {
  return getMinimalFeeForHeight(get_current_blockchain_height());
}
...
bool Core::handle_get_objects(NOTIFY_REQUEST_GET_OBJECTS_request& arg, NOTIFY_RESPONSE_GET_OBJECTS_request& rsp) { //Deprecated. Should be removed with CryptoNoteProtocolHandler.
...
bool Core::queryBlocks(const std::vector<Crypto::Hash>& knownBlockIds, uint64_t timestamp, uint32_t& resStartHeight, uint32_t& resCurrentHeight, uint32_t& resFullOffset, std::vector<BlockFullInfo>& entries) {
  LockedBlockchainStorage lbs(m_blockchain);
...
bool Core::queryBlocksLite(const std::vector<Crypto::Hash>& knownBlockIds, uint64_t timestamp, uint32_t& resStartHeight, uint32_t& resCurrentHeight, uint32_t& resFullOffset, std::vector<BlockShortInfo>& entries) {
  LockedBlockchainStorage lbs(m_blockchain);
...
bool Core::getPoolChanges(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds, std::vector<Transaction>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  getPoolChanges(knownTxsIds, addedTxs, deletedTxsIds);
  return tailBlockId == m_blockchain.getTailId();
}

void Core::getPoolChanges(const std::vector<Crypto::Hash>& knownTxsIds, std::vector<Transaction>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
...
bool Core::getPoolChangesLite(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds, std::vector<TransactionPrefixInfo>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
...
bool Core::handleIncomingTransaction(const Transaction& tx, const Crypto::Hash& txHash, size_t blobSize, tx_verification_context& tvc, bool keptByBlock, uint32_t blockHeight) {
  ...
  if (!check_tx_semantic(tx, keptByBlock, blockHeight)) {
  ...
  if (!check_tx_fee(tx, blobSize, tvc, keptByBlock, blockHeight)) {
    tvc.m_verification_failed = true;
    return false;
  }
...
bool Core::handle_incoming_tx(const BinaryArray& tx_blob, tx_verification_context& tvc, bool kept_by_block) { //Deprecated. Should be removed with CryptoNoteProtocolHandler.
  ...
  return handleIncomingTransaction(tx, tx_hash, tx_blob.size(), tvc, kept_by_block, blockHeight);
}
...
bool Core::getBlockReward1(size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins, uint64_t fee, uint64_t& reward, int64_t& emissionChange) {
  return m_currency.getBlockReward1(medianSize, currentBlockSize, alreadyGeneratedCoins, fee, reward, emissionChange);
}

bool Core::getBlockReward2(uint32_t blockHeight, size_t currentBlockSize, uint64_t alreadyGeneratedCoins, uint64_t fee, uint64_t& reward, int64_t& emissionChange) {
...
bool Core::check_tx_fee(const Transaction& tx, size_t blobSize, tx_verification_context& tvc, bool keptByBlock, uint32_t blockHeight) {
  ...
  if (!keptByBlock && !isFusionTransaction) {
    ...
    if (blockHeight >= CryptoNote::parameters::SOFT_FORK_HEIGHT_1 && fee < CryptoNote::parameters::MINIMUM_FEE_2)
    {
      ...
      ", minimum fee: " << m_currency.formatAmount(CryptoNote::parameters::MINIMUM_FEE_2);
      tvc.m_verification_failed = true;
...
bool Core::check_tx_semantic(const Transaction& tx, bool kept_by_block, uint32_t blockHeight) {
  ...
  if (blockHeight >= CryptoNote::parameters::SOFT_FORK_HEIGHT_1)
  {
    // number of signatures must match number of transaction inputs
    if (tx.inputs.size() != tx.signatures.size())
    {
      logger(ERROR) << "number of transaction signatures does not match number of inputs, rejected transaction id = " << getObjectHash(tx);
      return false;
    }
    ...
    for (size_t i = 0; i < tx.inputs.size(); ++i)
    {
      if (tx.inputs[i].type() == typeid(KeyInput)) {
        if (boost::get<KeyInput>(tx.inputs[i]).outputIndexes.size() != tx.signatures[i].size()) {
          logger(ERROR) << "transaction signatures count does not match outputIndexes count for input " 
            << i << ", rejected for transaction id = " << getObjectHash(tx);
          return false;
        }
      }
    }
  ...
  // Check that transaction extra size is not too large
  if (blockHeight >= CryptoNote::parameters::SOFT_FORK_HEIGHT_1)
  {
    if (tx.extra.size() > CryptoNote::parameters::MAX_TX_EXTRA_SIZE)
    {
      logger(ERROR) << "Transaction extra size is too large";
      return false;
    }
  }
